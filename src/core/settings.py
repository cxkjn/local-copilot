...
class Settings(BaseSettings):
    ...
    # Code assistant (code-assistant agent) configuration
    MAX_TOKEN_LIMIT: int = 8000
    CONTEXT_KEEP_RECENT_ROUNDS: int = 6
    REDIS_URL: str | None = None
    MEMORY_TTL_SECONDS: int = 3 * 24 * 3600
    PROJECT_ROOT: str = "."
    TOOL_WHITELIST: set[str] = Field(default_factory=set)
    HIGH_RISK_TOOLS: set[str] = Field(
        default_factory=lambda: {"WriteFile", "DeleteFile", "ShellExec", "RunCommand"}
    )
    CONTENT_BLOCKLIST: set[str] = Field(default_factory=set)
    BLOCK_RULE_SEMANTIC_CHECK: bool = True
    PROMPT_CACHING_ENABLED: bool = False
    MCP_SERVERS_JSON: str | None = None
    PLAN_MODE_MAX_SUBTASKS: int = 5
    PLAN_MODE_MAX_SUB_STEPS: int = 6
    PLAN_MODE_COMPLEXITY_THRESHOLD: int = 500
    ...
    POSTGRES_APPLICATION_NAME: str = "local-copilot"
