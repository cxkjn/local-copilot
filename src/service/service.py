...
async def invoke(user_input: UserInput, agent_id: str = DEFAULT_AGENT) -> ChatMessage:
    ...
    try:
        ...
        if "__interrupt__" in response:
            ...
            # The graph can end on a node that emits an empty "updates" event (e.g. the
            # terminal memory-harvest node returning {}), so fall back to the last full
            # state rather than failing.
            values = [resp for etype, resp in response_events if etype == "values"]
            if not values:
                raise ValueError(f"Unexpected response type: {response_type}")
            output = langchain_to_chat_message(values[-1]["messages"][-1])
