...
description: >-
  Draft replies to GitHub issues and pull requests on JoshuaC215/local-copilot
...
`https://github.com/JoshuaC215/local-copilot/issues/<NNN>`, PRs →
`https://github.com/JoshuaC215/local-copilot/pull/<NNN>`. Markdown form
...
1. Use `gh issue comment <n> --body-file <file>` / `gh pr comment <n> --body-file <file>`
   ...
   be inside the repo.
2. **Always pass `-R JoshuaC215/local-copilot` on every `gh` write.** `gh issue
   comment` / `gh pr comment` infer the repo from the current directory's git remote, so they
...
3. **Know whether each number is an issue or a PR before posting** — `gh issue comment` and
   `gh pr comment` are **not** interchangeable (using the wrong one errors). When the draft
   report doesn't already label it, check first:
   ```sh
   gh api repos/JoshuaC215/local-copilot/issues/<n> \
...
4. **Do not append any attribution, footer, "Generated by", or co-author line to comment
...
5. Confirm identity first with `gh api user --jq .login` → must be `JoshuaC215`. If it isn't
...
6. To **comment then close** an issue, chain the close after the comment succeeds:
   `gh issue comment <n> -R JoshuaC215/local-copilot --body-file <file> && gh issue close <n> -R JoshuaC215/local-copilot`.
7. After posting, record the comment URL back into the report's `Status` (☑️ Posted).
